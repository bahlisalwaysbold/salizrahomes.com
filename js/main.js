...
/* ---------- Skeleton upgrade: accessible mobile nav ---------- */
(function () {
  var toggle = document.querySelector('.nav-toggle');
...
/* ---------- Skeleton upgrade: FAQ accordion ---------- */
document.addEventListener('DOMContentLoaded', function () {
...
document.addEventListener('DOMContentLoaded', function () {
  var profilePage = document.querySelector('.cp-hero');
  if (!profilePage) return;

  // Animated metric counters — kept deliberately light for mobile.
  ...
  function runCounts() {
    ...
    countEls.forEach(function (el) {
      ...
      var duration = target > 100000 ? 1100 : 850;
      function tick(now) {
  ...
  var statStrip = document.querySelector('.cp-stat-strip');
  ...
  // Add a gentle focus ring when users arrive via keyboard.
  document.querySelectorAll('.cp-doc-card a, .cp-download-btn').forEach(function (link) {
    link.addEventListener('focus', function () { link.closest('.cp-doc-card, .cp-dossier-card, .cp-hero-actions')?.classList.add('keyboard-focus'); });
    link.addEventListener('blur', function () { link.closest('.cp-doc-card, .cp-dossier-card, .cp-hero-actions')?.classList.remove('keyboard-focus'); });
  });
});
